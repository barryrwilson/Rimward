...
import { detailBuilder, rng, box, cyl, sphere, hemi, torus, cone, ribBands, windowRow, windowGrid, portholeRing, panelSkin, truss, railing, bridge, airlock, panelPatches, pipeRun, antenna, ladder, radiatorPanel, lampString, crate } from './station-detail.js'; // waves 43-44: merged-geometry detail kit
 ...
 * generalising the wave-37 D4 ship ruling to stations. Wave 44 keeps that
 * machinery and rebuilds the SCULPT: the wave-43 pass scattered six smooth
 * drums across a 58-unit span with air between them, and read as unrelated
 * objects rather than a station. The reference art
 * (docs/FactionExamples/03-freehold-compact-station.png) is a PACKED pile —
 * modules interpenetrate, every surface is tiled with small panel plates, and
 * airlock collars and catwalks stitch the whole mass together.
 *
 ...
 *
 * Weathering ladder (wave 44): structure colours are `fhWeather(paletteHex, i)`
 * over SHADES [1.0, 0.86, 0.72, 0.6] — the reference art is heavily weathered
 * and one flat tone per role read as plastic. scripts/boot-test.mjs recomputes
 * the identical product set with the same Math.round arithmetic and requires
 * exact membership, so every shade must derive from a BASE FACTION_STYLE
 * colour, never from an already-weathered one (round(round(c * 0.86) * 0.86)
 * is not round(c * 0.72)).
 *
 * Envelope: U.DOCK_RANGE is 45, so the mesh bounding box stays inside
 * |x|,|z| <= 32 and y in [-26, 33]; the stationRecord beacon owns y = 31.
 */

/** Weathering ladder — mirrored bit-for-bit by the wave-43 palette pin. */
const FH_SHADES = [1.0, 0.86, 0.72, 0.6];
function fhWeather(hex, i) {
  const f = FH_SHADES[i];
...
function buildFreeholdStation(ctx, systemId, def) {
  ...
  ringGroup.add(new THREE.Mesh(ringGeos.ringHull, hullMat));
  ringGroup.add(new THREE.Mesh(ringGeos.ringGlow, lightMat));
  ringGroup.add(new THREE.Mesh(ringGeos.ringGlaze, glazeMat));
  group.add(ringGroup);
