...
for(let i=1;i<=count;i++)await runLive('natural-'+i,async h=>{
  ...
  async function followIncidental(s){
    ...
    if(!collected)throw Error('Natural incidental opportunity could not be collected during bounded public-bearing pursuit');
  ...
  async function navigate(label,action,pred,seconds){const a=await observe(),wall=Date.now(),earningWorld=result.timing.collectionDispositionWorldSeconds,earningWall=result.timing.collectionDispositionWallSeconds;await action();const s=await wait(pred,seconds,label,async s=>{sample(s);if(await followIncidental(s)){s=await observe();if(!pred(s)){if(s.flags.docked)await act('undock');await action();}}});result.timing.navigationWorldSeconds+=s.t-a.t-(result.timing.collectionDispositionWorldSeconds-earningWorld);result.timing.navigationWallSeconds+=(Date.now()-wall)/1000-(result.timing.collectionDispositionWallSeconds-earningWall);return s;}
  ...
  await navigate('initial-dock',()=>act('approachDock'),s=>s.flags.docked,150);
  ...
  await navigate('Veridian-dock',()=>act('approachDock'),s=>s.flags.docked,180);
  ...
  if(result.firstActualOpportunity&&!result.incidental?.complete)throw Error('Natural opportunity observed without completed disposition');
