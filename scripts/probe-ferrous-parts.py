"""Smoke-probe the Ferrous Hegemony shared construct modules under Blender.

Why this exists: `ship_builders/ferrous/armour.py` and
`ship_builders/ferrous/hardware.py` will be consumed by six class
files. A defect in one shared construct is otherwise diagnosed six times.
This probe runs every public construct at every detail level before ace,
heavy and frigate exist.

Run:
    "C:/Program Files/Blender Foundation/Blender 5.2/blender.exe" -b \
        -P scripts/probe-ferrous-parts.py
...
clean. Default rib_flare long-axis span is checked against the
...
from ship_builders.ferrous import armour as ar
from ship_builders.ferrous import hardware as hw
from ship_builders.ferrous import surface as sf
...
# Dummy iron-wedge stations for surf-callback self-trim (class authors
# will own a real list). Used only to prove the factories return 0.0
# off-section.
STATIONS = [
    sf.fair(-5.0, 0.40, 0.36, 0.0),
    sf.fair(-2.0, 0.62, 0.50, 0.0),
    sf.fair(0.0, 0.68, 0.54, 0.0),
    sf.fair(2.5, 0.52, 0.46, 0.0),
    sf.fair(5.0, 0.36, 0.32, 0.0),
]
...
def cases():
    ...
    # ---- armour.py --------------------------------------------------------
    out.append(('armour.rib_flare', lambda p, g, hm, gm, d: ar.rib_flare(
        p, 'rf', hm, (0.0, 0.0, 0.0), detail=d)))
    out.append(('armour.rib_run', lambda p, g, hm, gm, d: ar.rib_run(
        p, 'rr', hm, (0.0, 0.0, 0.0), count=28, detail=d)))
    out.append(('armour.citadel_plate', lambda p, g, hm, gm, d: ar.citadel_plate(
        p, 'cp', hm, (0.0, 0.4, 0.0), detail=d)))
    out.append(('armour.armour_course', lambda p, g, hm, gm, d: ar.armour_course(
        p, 'ac', hm, (0.0, 0.0, 0.0), detail=d)))
    out.append(('armour.chamfered_course', lambda p, g, hm, gm, d: ar.chamfered_course(
        p, 'cc', hm, (0.0, 0.0, 0.0), detail=d)))

    # ---- hardware.py ------------------------------------------------------
    out.append(('hardware.battery_module', lambda p, g, hm, gm, d: hw.battery_module(
        p, g, 'bm', hm, gm, (0.0, 0.2, 0.0), facing='up', detail=d)))
    out.append(('hardware.turret_rail', lambda p, g, hm, gm, d: hw.turret_rail(
        p, g, 'tr', hm, gm, (0.0, 0.2, 0.0), count=4, axis='z', detail=d)))
    out.append(('hardware.rescue_pannier', lambda p, g, hm, gm, d: hw.rescue_pannier(
        p, g, 'rp', hm, gm, (0.8, -0.1, 0.2), detail=d)))
    out.append(('hardware.rescue_hatch', lambda p, g, hm, gm, d: hw.rescue_hatch(
        p, g, 'rh', hm, gm, (0.0, -0.4, 0.0), face='-y', detail=d)))
    for n in (2, 4, 6, 8):
    ...
    out.append(('hardware.hangar_berth', lambda p, g, hm, gm, d: hw.hangar_berth(
        p, g, 'hb', hm, gm, (0.0, -0.2, 0.4), facing='starboard', detail=d)))
    out.append(('hardware.recognition_band', lambda p, g, hm, gm, d: hw.recognition_band(
        p, 'rb', hm, (0.0, 0.1, -1.8), detail=d)))
    out.append(('hardware.honor_plate', lambda p, g, hm, gm, d: hw.honor_plate(
        p, 'hp', hm, (0.3, 0.2, -1.4), detail=d)))
    ...
    out.append(('g2.rib_flare span', lambda p, g, hm, gm, d: _probe_g2_flare(p, hm, d)))
    return out
...
def _probe_surf():
    ...
    if sf.RIB_FLARE_REACH < G2_FLOOR:
        raise RuntimeError('default RIB_FLARE_REACH must be >= 15 % of cutter length')
    if sf.ARMOUR_RIB_T < VOXEL * 0.5:
        raise RuntimeError('ARMOUR_RIB_T is too thin to read')
    if sf.LAMP_SPACING != 1.20:
        raise RuntimeError('LAMP_SPACING must stay HUMAN.lampGap 1.20')
    if sf.RESCUE_PANNIER != (0.45, 0.62, 1.20):
        raise RuntimeError('RESCUE_PANNIER must stay the pilot human size')
    if sf.PORT_LIGHT != (0.22, 0.10, 0.04):
        raise RuntimeError('PORT_LIGHT must stay the pilot human size')
    if sf.PORT_SPACING != 0.35:
        raise RuntimeError('PORT_SPACING must stay the pilot human pitch')
    if sf.STATUS_SLIT != (0.10, 0.05, 0.04):
        raise RuntimeError('STATUS_SLIT must stay the pilot human size')
    if sf.RESCUE_LAMP != (0.12, 0.06, 0.06):
        raise RuntimeError('RESCUE_LAMP must stay the pilot human size')
    return []
...
def _probe_g2_flare(parts, hull_mat, detail):
    if detail != 3:
        return []
    objs = ar.rib_flare(parts, 'g2rf', hull_mat, (0.0, 0.0, 0.0), detail=3)
    tris, (lo, hi), bad = measure(objs)
    span = _long_span(lo, hi)
    if span < G2_FLOOR:
        raise RuntimeError('rib_flare default long-axis span %.3f < %.3f'
                           % (span, G2_FLOOR))
    return objs
...
def main():
    ...
    failures = []
    print('== Ferrous Hegemony shared-construct smoke probe')
    ...
    print('')
    if failures:
        print('FERROUS PART PROBE: %d PROBLEM(S)' % len(failures))
        for line in failures:
            print('  - ' + line)
        sys.exit(1)
    print('FERROUS PART PROBE: ALL CONSTRUCTS CLEAN')
