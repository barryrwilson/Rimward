...
const materialsRoot = fileURLToPath(new URL('../public/assets/ships/materials/', import.meta.url));
...
const factions = await readdir(materialsRoot, { withFileTypes: true });
let count = 0;
for (const faction of factions.filter((entry) => entry.isDirectory())) {
  for (const role of ['trader', 'pirate']) {
    const path = join(materialsRoot, faction.name, role);
...
console.log(`Encoded ${count} KTX2 ship atlas maps.`);
