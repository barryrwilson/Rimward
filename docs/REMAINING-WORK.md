...
|---|---:|---|---|---|
| [AGENT-071](https://github.com/barryrwilson/Rimward/issues/71) | P1 | Implemented; focused tests/build pass; full boot and independent/live verification pending | Save completed dock delivery passes (including passenger replacements), market trades, accepted jobs and ordinary paid services after their full mutations. Failed autosave requests retain a session-only retry at the berth through the existing encounter/mid-jump gates; storage failures surface their actual reason. No save-schema change. | 2026-09-07 courier rollback: arrival autosave preceded delayed settlement and later docked transactions. `npm run test:dock-persistence` covers fresh-process restore/no repeated payout, complete agreements, refusals, storage failure and gated retries. Historical passenger rollback despite redock remains unexplained by the available trace; no single cause is claimed. |
