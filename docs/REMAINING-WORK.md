...
| [REL-003](https://github.com/barryrwilson/Rimward/issues/33) | P1 | Workflow and policy implemented; final hosted dry run follows merge | Protect `master` with the exact `Build` and `Boot harness` checks. A manually dispatched release candidate accepts one full commit SHA, verifies the checked-out tree matches it, runs every release gate without hiding later results, uploads bounded evidence, and emits one final verdict. REL-001, REL-002, and REL-004 are integrated; the remaining acceptance evidence is one passing final-SHA run from `master` after this workflow merges. | [Release-readiness audit](https://github.com/barryrwilson/Rimward/issues/33) |
