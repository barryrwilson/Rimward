...
| A pirate may claim the crew and the hull of a yielded trader | [#147](https://github.com/barryrwilson/Rimward/issues/147) | Implemented: `src/game/prize.js` + the boarding loop in `npc.js`. When a trader yields with its crew aboard (`jettison` / `cutEngines`) to an NPC pirate (world causer, #99), `capitulate` rolls ONE choice from the pirate's persisted taste and temper (`PRIZE` in `state.js`): every pirate record rolls a taste once (`rec.taste`, weights cargo 0.5 / crew 0.3 / hull 0.2, lazy like `temper`, sanitized on restore) — a cargo raider (temper-0 → cargo only), a slaver (crew 0.35 + 0.35 × temper) or a prize-crew captain (hull 0.30 + 0.35 × temper) — and its boarding line reads its trade. Cargo only is the #146 run plus the #151 scoop. Crew or hull: the trader heaves to (engines dark, 3 u/s way on, instance-only `ai.heldBy`), the pirate closes to contact + `boardRange` and holds `boardSeconds`, then takes the crew as ONE wave-60 survivor row (`{ commodity: 'survivor', units: 1, faction, source: 'other', name }`) on its own `rec.cargo`. Crew only: the hull is a #148 derelict with reason `crewTaken`. Hull too: the trader record ends `captured` (traffic retires the hull; it leaves the finite population) and `rec.prize = { v, id, name, classKey, takenAt, system }` rides the pirate to the #151 fence run, where the sale (`fencePrize`) pays goods at `ECON.fenceRate`, captives at `TRAFFIC_LIST_UU.other` (a Gilded Chain station) or `PRIZE.captiveRansom` elsewhere, and the hull at `ECON.hotHullFence` × `hullPrizeValue(classKey)`; `npcFenced` gains `captives` / `hull`. Receipt `npcPrizeTaken` (off the agent ring, causer `world`, pays no fear/bounty/standing). A hit on the boarder, a hit on the prize by anyone but the boarder, the player's lock on the prize (#123), a boarder that vanishes, or `heaveSeconds` elapsing breaks the boarding off and the trader keeps its crew and runs (#146). A job quarry (any job whose `target` / `targetId` names the record) and a player-caused break are never boarded; a `crewPods` break has no crew to take. Captives aboard a broken pirate spill as survivor pods with their identity. `save.js` sanitizes `rec.prize`. Pinned in `npm run test:prize` (58 pins) and run as a checked child inside `npm run test:boot`; the #146/#148/#151/#123 fixtures pin `temper: 0, taste: 'cargo'` (cargo only). Open questions decided: a named quarry is excluded from prize sale; a prize sale does not feed the market (only the fenced goods do). Parked: a visible prize crew flying the hull to the station (the captured hull despawns at boarding completion, as the issue proposes); the pirate-faction Q-ship re-entry; a boarding that survives a save/restore or a range cull (instance-only: the trader comes back enroute). |
| An unclaimed derelict is claimable and folds away after 30 minutes | [#148](https://github.com/barryrwilson/Rimward/issues/148) | Implemented: `src/game/derelict.js`. A `crewPods` yield flips the record to state `derelict` with a JSON-plain `rec.derelict` (since, due = since + `DERELICT.foldAfter` 30 min, a salvager `claimAt` rolled once, dead-stick position/coast, hull left, the aftermath wreck id); the escape plan is dropped and the manifest emptied. The player claims it through the #74 recovery board (same card, titled "Recovery: derelict hull", same marker pod and receipts; scooping the marker resolves it `recovered`); a local salvager takes it under tow at `claimAt` (`salvaged`, deferred while the player holds an accepted contract on it or keeps it locked); unclaimed it folds at `due` (`expired`). Resolutions take the existing terminal words (`dead` / `captured`) so traffic, bank advance, migration and quarry checks need no new case. Traffic re-instantiates a culled derelict as the same dead hull; `tickDerelicts` sweeps every bank at 1 Hz so the timer survives the cross-system fold; `save.js` sanitizes the blob (corrupt → dropped, record ended). HUD bracket reads DERELICT. Pinned in `npm run test:derelict` (45 pins) and run as a checked child inside `npm run test:boot`; live check 2026-09-12 (yield → derelict, bracket, board card, marker scoop → hull leaves, clean console). Parked: a visible salvage tug (the NPC claim is abstract: one Echo line and the hull is under tow); the salvage hail (H) on a derelict is unchanged (it keys on `disabled`). |
...
| An NPC pirate scoops the pods it took and sells the haul at a fence | [#151](https://github.com/barryrwilson/Rimward/issues/151) | Implemented: `spillShipCargo` (npc.js) tags every pod of a WORLD-caused spill with the breaking pirate's record id (`pod.spilledBy`; a player-caused break leaves them untagged). When its prize yields or dies the pirate opens a bounded `ai.scoop` phase (`PIRATE_HAUL` in `state.js`: 90 s, 12 u collect / 40 u magnet), flies to its own pods and merges them into its record manifest (`rec.cargo`, the same array the live state holds), then hunts again; pods inside the player's magnet reach (`SCOOP_RANGE` × 3) are the player's, and any hit drops the scoop. A hold at `fenceAt` (50 % of `cargoHoldFor`) sends it on an `ai.fence` run to the local station hold (`src/game/pirate-haul.js`): 12 s at the pad, then every priced row sells at a per-sale `ECON.fenceRate` roll against that system's `world.markets` table, `commitMarketSupply` takes the units, and `rec.credits` (JSON-plain, sanitized on restore) banks the pay. Receipts `npcPodCollected` / `npcFenced` (never a player `podCollected`; not on the agent ring) plus one comm line each. Pinned in `npm run test:pirate-haul` (38 pins) and run as a checked child inside `npm run test:boot`. Parked: a pirate scooping a spill it did not cause (a wreck field, a player jettison); a purse that the pirate ever spends. |
