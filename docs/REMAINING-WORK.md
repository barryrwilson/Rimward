...
|---|---:|---|---|---|
| [#62](https://github.com/barryrwilson/Rimward/issues/62) | P1 | Functional PASS; owner exceptions approved; build/bundle/artifact verification PASS; final policy review pending | Reactive defense runs inside the existing bounded combat lease, responds to visible warnings/hits without requiring attacker identity, and preserves human takeover, collision priority and stable withdrawal completion. No standalone helm or persistent stance. | [Design](AgentReactiveDefenseDesign.md), [verification record](playtests/2026-09-09-issue-62-reactive-defense.md) and [approved exact exceptions](releases/issue-62-measured-exception.md). Independent source/probe/functional review passes `30c0b280`; F1–F5 documentation precision fixes are `95023918`. Runtime remains `97c5ea1`; 19 defense groups, 33 combat-intent groups, boot, controlled 5/15/30-second and clean natural G1 evidence retain their stated limits and failures. Owner approved exact #62 bytes and separately the measured startup result, plus opening the PR. On activation `f09a84cf`, actual build and bundle report PASS through the exact exception with false raw byte flags; all 447 emitted files, canonical manifest and runtime source match the approved candidate, and the browser boundary passes with only `three`. Historical overage remains 21,216 minified / 6,222 gzip bytes; the two-of-five raw startup FAIL is separately approved, not relabeled. Global limits unchanged. Final policy/documentation review is pending; no issue closure, merge, deployment or full release readiness claimed. |
...
| [TRADE-002](https://github.com/barryrwilson/Rimward/issues/55) | P1 | Implemented and verified locally; unpublished | [Approved design](MarketLiquidityProgressionDesign.md) implements 160 bulk / 20 other stock and a 20-minute simulation refill with scoped marketSupply persistence. Owner accepted measured 9m10.8s progression and exact byte/startup exceptions. Independent source/security and policy/security reviews pass, as do boot, live checks and the ordinary build; all 447 emitted files match the tested candidate. | [Implementation evidence and caveats](playtests/2026-09-09-issue-55-market-liquidity.md), including raw misses, approved exceptions, save recovery and known limitations. |
...
- **2026-09-09 design status:** Fresh corrected-market evidence and a concrete
  supply-only recommendation are in [MarketLiquidityProgressionDesign.md](MarketLiquidityProgressionDesign.md).
  The owner approved the design and scoped persistence field on 2026-09-09.
  Source is implemented at `e3761b76`. Focused tests and boot pass; three fresh
  progression runs and earned full-load freight are recorded in the
  [implementation playtest](playtests/2026-09-09-issue-55-market-liquidity.md).
  Independent source/security review passes with nonblocking notes.
  The owner accepted the measured pacing and exact byte/startup exceptions.
  Ordinary build/report and complete artifact equivalence pass. Independent
  policy/security review also passes on `8efc2cba`. Work is complete locally;
  publication, merge, deployment and GitHub issue closure have not occurred.
...
| [OPT-003](https://github.com/barryrwilson/Rimward/issues/10) | P3 | Reassessment complete locally; independent review tracked with the task handoff. 32 seeded starter simulations, both rendered beginner origins through 240s, and an adverse live pursuit/return/re-entry check support retaining the shipped 180s starter grace and 300u station law zone. No additional bubble justified by the sampled pressure. Sun-core losses in unsteered starts are separately documented and censored; no gameplay change. [Evidence and reproducible probes](Issue10StarterPacingEvidence.md). |
| [OPT-004](https://github.com/barryrwilson/Rimward/issues/11) | P3 | Implemented; verification and final independent artifact QA PASS on `a0d42082d250b763038bd52e53eb6e389c31b16b`. Reproduced Settings/pause fire leaks and cleared physical holds at ownership boundaries; fresh flight fire still breaks hail demands. Focused 256/256, full boot, live ownership 41/41 and actual focus 5/5 pass. Claude independently confirmed source/bundle identity; the documented minor keyboard-override caveat remains. Build/report pass via the owner's exact byte exception. Hosted Build, Boot harness and OPT-001 checks pass in [PR #92](https://github.com/barryrwilson/Rimward/pull/92), runs [34410624390](https://github.com/barryrwilson/Rimward/actions/runs/34410624390) and [34410624215](https://github.com/barryrwilson/Rimward/actions/runs/34410624215). Global limits/startup requirements unchanged; no merge/deployment claimed. See [Issue11FireHeldEvidence.md](Issue11FireHeldEvidence.md). |
| [OPT-005](https://github.com/barryrwilson/Rimward/issues/12) | P3 | Implemented; independent source/security/behavior review PASS. A 303-board census reproduced 114 trade and 291 survey twin pairs; the focused correction removes those twins while preserving distinct targets, cargo, destinations, locked rewards, accepted contracts, and the intentional independent passenger parties from #73. Full boot, focused census, survey navigation, and live boards pass. Ordinary build/report pass under the owner's [exact byte and separate startup exceptions](releases/issue-12-measured-decision.md); global limits remain unchanged. Final policy review is recorded in the task evidence. No publication, merge, or issue closure claimed. See [Issue12MissionDuplicateCensus.md](Issue12MissionDuplicateCensus.md). |
