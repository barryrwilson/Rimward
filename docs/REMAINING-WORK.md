...
|---|---:|---|---|---|
| [#67](https://github.com/barryrwilson/Rimward/issues/67) | P2 | Implemented; focused tests pass; build, full boot and the live probe are pending execution on the exact commit; independent review pending | Make the displayed capitulation state and the interaction actually offered agree. One shared player-visible classifier (`src/game/hail-offer.js`) answers the HUD target bracket/prompt, the KeyH miss toast and `observe().targets.current`, on two independent axes. **Persistent encounter state** — `salvage` / `yielded` / `willing` / `no-hail` — belongs to the hull and is never erased by an overlay, a calm window or a card open on another ship. A completed surrender reads `YIELDED · NO HAIL CLAIM`; a hull that is merely `willing` keeps its `BARGAINING`/`CAPITULATE` band and adds `NO TERMS`; a disabled hull keeps `DEAD IN SPACE`. The `willing` name is deliberately neutral so it stays true with or without a live card, and it is a morale reading, never a reward. **Transient blocker** — `busy` / `surface` / `overlay-chart` / `overlay-berth` / `geometry` / `range` / `calm` — follows `hail.js`'s real KeyH precedence for every live-ship lock, so the published `blocked`/`reason`/`next` and the toast the key actually emits can never disagree: an open overlay outranks range and morale for every state, an open card suppresses both the refusal token and the `NO TERMS` clause and points at the card instead, and range/geometry/calm gate only the salvage card they can really hold shut — an already-yielded or willing hull keeps its own specific reason under calm. The H prompt is now shown only when the key really opens a card, so an intact `CAPITULATE` trader no longer advertises an action that answers "no hail". The miss toast name obeys the wave-31 Q-ship cover and Mk II scanner rule. Scope stayed on state and feedback: no combat, economy, resolve, reward, persisted field or schema version change, no new negotiation verb, no change to KeyH's own gating (the classifier reads the same `playSurfaceBlocked`/`settingsOwnsScreen` helpers `hail.js` reads plus the raw pause flag, because KeyH cannot reach `hail.js` at all while paused: `main.js` skips the whole update loop, `controls.js` returns early from keydown, and `act()` refuses with the `paused` token), and no cargo/AI/hidden-identity disclosure — the classifier never reads the manifest and never calls `hailApi.peek()`, so #66's one authoritative peek per observation is preserved along with #63 earned attribution. | 2026-09-08 issue #67: intact Claim Wren rec-15 at 176 u and Red Marlow rec-9 at 220 u showed `resolveBand` capitulate but the queued hail ended `no-hail` with no usable card. Regressions: `npm run test:capitulation-feedback` (187 pins, including the adversarial overlap matrix that pins the shared refusal against the REAL KeyH result for overlay/calm/open-card/range/geometry across every state). `npm run test:hail-identity`, `test:first-scare`, `test:agent-schema` and `test:agent-hardening` pass. Live browser probe `npm run test:capitulation-live` (`scripts/issue-67-live-probe.mjs`, L1-L9: fresh Marked origin, rendered `NO TERMS` bracket with no useless prompt, an earned yield through the public `demandRansom`, repeated H with no duplicate reward, own/unrelated cards, empty holds, salvage, out of range and the chart overlap) is written and syntax-checked but NOT yet executed — it, `npm run build` and full `npm run test:boot` are to be run on the exact commit. |
