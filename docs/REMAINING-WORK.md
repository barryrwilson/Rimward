...
|---|---:|---|---|---|
| [#74](https://github.com/barryrwilson/Rimward/issues/74) | P2 | Implemented; automated checks pass; final live and independent QA gates pending | Jobs now gives a stock Greenhand the same salvage guidance through UI and public observations: whether confirmed local recovery is posted, why it can be absent, and a bounded station-to-gate search followed by the next dock's board or other work. Accepted real-wreck recovery exposes a flight marker and matching public bearing/range, requires two free hold units, credits only its own pod, preserves the original deadline through restore, and retains collected/payment state. Expired or legacy unbounded leads fail explicitly. Foreign cards direct return to the issuing dock and quote its rate. Actual NPC aftermath and payouts remain unchanged; no guaranteed spawn, income, new persisted field, control or equipment. | [Salvage onboarding decision and verification contract](SalvageOnboardingDesign.md), from the [2026-09-07 salvager playtest](playtests/2026-09-07-salvager-results.md). Final UI build passes at 1,799,979 B / 524.30 KiB gzip with unchanged budgets; focused recovery tests pass 23 pins, including generated foreign-dock quote parity. Local full boot and existing regressions pass before the documented final UI-only corrections. Three fresh native-RNG profiles, supplemental browser checks and independent QA are still completing at this handoff; final evidence is tracked separately in `out/issue-74-evidence/`. Controlled injected destruction-event fixtures do not establish natural availability or profitability. |
