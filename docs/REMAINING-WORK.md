...
|---|---:|---|---|---|
| [#67](https://github.com/barryrwilson/Rimward/issues/67) | P2 | Implemented; focused tests pass; build, full boot and the live probe are pending execution on the exact commit; independent review pending | Make the displayed capitulation state and the interaction actually offered agree, **on the screen space that already exists**. One shared player-visible classifier (`src/game/hail-offer.js`) answers the HUD target bracket/prompt, the KeyH miss toast and `observe().targets.current`, on two independent axes. **Persistent encounter state** — `salvage` / `yielded` / `willing` / `no-hail` — belongs to the hull and is never erased by an overlay, a calm window or a card open on another ship. It is shown by changing the word on the EXISTING resolve line: `BAND_LABEL.capitulate` now reads `WILLING TO YIELD` (morale, not a completed act), a finished surrender reads `YIELDED`, a disabled hull reads `DEAD IN SPACE`, and `BARGAINING`/`SHAKEN`/`DEFIANT` are unchanged. Owner call in the UI follow-up: **no dedicated row and no passive clause** — the target meta line returns to faction, distance and the concealed-mounts mark only, with no `NO TERMS`, `NO HAIL CLAIM` or `CLOSE TO SALVAGE` suffix, because inactive notices cost screen space better spent elsewhere. The `data-band` dataset vocabulary and the Wolfeye numeric-resolve gates are untouched. **Transient blocker** — `busy` / `surface` / `overlay-chart` / `overlay-berth` / `geometry` / `range` / `calm` — follows `hail.js`'s real KeyH precedence for every live-ship lock, so the published `blocked`/`reason`/`next` and the toast the key actually emits can never disagree: an open overlay outranks range and morale for every state, an open card suppresses the refusal token and points at the card, and range/geometry/calm gate only the salvage card they can really hold shut — an already-yielded or willing hull keeps its own specific reason under calm. Feedback is otherwise on demand only: the H prompt appears only when the key really opens a card, and a deliberate H press explains the refusal — `already yielded, no further terms` (public `next`: "They have already yielded. No further terms to negotiate.") or `no terms offered`. The miss toast name obeys the wave-31 Q-ship cover and Mk II scanner rule. Scope stayed on state and feedback: no new DOM row, CSS, gauge or control; no combat, economy, resolve, reward, persisted field or schema version change; no new negotiation verb; no change to KeyH's own gating; and no cargo/AI/hidden-identity disclosure — the classifier never reads the manifest and never calls `hailApi.peek()`, so #66's one authoritative peek per observation is preserved along with #63 earned attribution. | 2026-09-08 issue #67: intact Claim Wren rec-15 at 176 u and Red Marlow rec-9 at 220 u showed `resolveBand` capitulate but the queued hail ended `no-hail` with no usable card. Regressions: `npm run test:capitulation-feedback` (188 pins, including the adversarial overlap matrix pinning the shared refusal against the REAL KeyH result for overlay/calm/open-card/range/geometry across every state, and source pins that no passive clause text remains). `npm run test:hail-identity`, `test:first-scare`, `test:agent-schema` and `test:agent-hardening` pass. Live browser probe `npm run test:capitulation-live` (`scripts/issue-67-live-probe.mjs`, L1-L9: fresh Marked 350 UU / fear 15 receipt taken before any fixture, the rendered `WILLING TO YIELD` line with a clean meta line and no useless prompt, an earned yield through the public `demandRansom` flipping that same line to `YIELDED`, three retained repeated-H checks with no duplicate reward, own/unrelated cards, empty holds, salvage, a real >600 u out-of-range refusal and the chart overlap) is written and syntax-checked but NOT yet executed — it, `npm run build` and full `npm run test:boot` are to be run on the exact commit. |
