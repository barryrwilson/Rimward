...
|---|---:|---|---|---|
| [#61](https://github.com/barryrwilson/Rimward/issues/61) | P1 | Implemented on isolated branch; verification pending | A bounded, explicitly renewed target-specific combat intent shares controls ownership with raw leases. Pursuit, firing passes and repositioning continue between decisions; target terminals, human takeover and lifecycle changes release control. #62 defensive reactions remain separate. | [Combat intent contract](AgentCombatIntentDesign.md). Initial focused tests pass; live/boot/independent QA and the measured production byte-budget failure remain release gates. |
