...
|---|---:|---|---|---|
| [#67](https://github.com/barryrwilson/Rimward/issues/67) | P2 | Implemented; focused tests, build and full boot pass; live browser verification and independent review pending | Make the displayed capitulation state and the interaction actually offered agree. One shared player-visible classifier (`src/game/hail-offer.js`) now answers the HUD target bracket/prompt, the KeyH miss toast and `observe().targets.current`, on two independent axes: a persistent per-hull encounter state (`salvage` / `yielded` / `unanswered` / `no-hail`) and a transient blocker (`geometry` / `range` / `surface` / `overlay-chart` / `overlay-berth` / `busy` / `calm`). A completed surrender reads `YIELDED · NO HAIL CLAIM` and answers H with `yielded`; a hull that is merely willing keeps its `BARGAINING`/`CAPITULATE` band, adds `NO TERMS` and answers H with `no-answer`; a disabled hull keeps `DEAD IN SPACE` and the salvage card. The H prompt is now shown only when the key really opens a card, so an intact `CAPITULATE` trader no longer advertises an action that answers "no hail". An unrelated open card, an overlay, a pause or a calm window can no longer erase the selected target's own outcome, and the `NO TERMS` clause is suppressed while any card is offering terms. Session calm no longer conceals an already-yielded hull; the miss toast name now obeys the wave-31 Q-ship cover and Mk II scanner rule. Scope stayed on state and feedback: no combat, economy, resolve, reward, persisted field or schema version change, no new negotiation verb, and no cargo/AI/hidden-identity disclosure — the classifier never reads the manifest. #66 speaker identity (single authoritative `peek()`) and #63 earned attribution are preserved. | 2026-09-08 issue #67: intact Claim Wren rec-15 at 176 u and Red Marlow rec-9 at 220 u showed `resolveBand` capitulate but the queued hail ended `no-hail` with no usable card. Regressions: `npm run test:capitulation-feedback` (101 pins — low morale vs completed yield, own and unrelated open card, real KeyH misses with public parity, laden vs empty wreck indistinguishable, calm/overlay/paused/range/invalid geometry, fresh fearful 350 u origin with no reward movement, scanner/cover tiers, single-peek #66 pin). `npm run test:hail-identity`, `test:first-scare`, `test:agent-schema`, `test:agent-hardening`, `npm run build` and full `npm run test:boot` pass. Live browser verification is owned by the parent task. |
