...
| [OPT-005](https://github.com/barryrwilson/Rimward/issues/12) | P3 | Implemented; final verification/release gate pending. A 303-board census reproduced 114 trade and 291 survey twin pairs; the focused correction removes those twins while preserving distinct targets, cargo, destinations, locked rewards, accepted contracts, and the intentional independent passenger parties from #73. All seven renewable slot families and supplemental bounty/recovery/chain mechanisms are documented in [Issue12MissionDuplicateCensus.md](Issue12MissionDuplicateCensus.md). The candidate requires measured byte and startup decisions; the existing build policy remains unchanged. |
