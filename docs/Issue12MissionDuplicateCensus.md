...
Both final census runs and the full boot test pass. The live probe exercises rendered boards, acceptance/expiry/refill, legacy slot 1, and double-passenger settlement. Independent Claude review returned source/code, security, and behavior **PASS** on `5c69b85a08efe2705f1b400e1390a64257cd16db`, independently rerunning the census and survey-navigation regression and matching the runtime evidence hash. Browser interaction used the public API and documented fixtures; the reviewer inspected its evidence without claiming to drive the browser.

The ordinary build reports **1,825,869 minified / 545,409 gzip bytes**, **914 / 311 bytes** above the prior exact artifact exception. The owner's conditional approval was fulfilled by the passing source review; ordinary build/report now pass using only the [exact-artifact byte exception](releases/issue-12-measured-decision.md). Cold startup's **9,722.9 ms median / 10,735.9 ms slowest** exceeds the **8,000 ms** target and has a separate approved exception. Global limits, the exact matcher, and runtime source remain unchanged. Final policy review is recorded in the coordinating task evidence. No publication or merge is claimed. Reverting the bounded station change restores previous generation; no save migration is needed.

Nonblocking review notes remain scoped out: distinct recovery wrecks still share generic card wording; chain SKU hints can occupy a second reward row but chains have separate census fixtures; surveys at different issuing docks are not treated as same-board twins. No follow-up issue was created.
