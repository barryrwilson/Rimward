# Current NPC turrets inventory (Wave 97)

**Wave:** 97. Design only. No `src/` edits.  
...
**Not this note:** chaff, an incoming turret gauge, Digit theft, a new `WEAPONS` row, TGT-03 remaining, BIO-05 remaining, invented fire percent / UU.
...
[`docs/NpcMissilesDesign.md`](../../../docs/NpcMissilesDesign.md) still freezes **no NPC player-style `auto` turret**. **Do not edit that file.** Wave 68 player `auto` and Wave 83 pirate+ace darts stay shipped. This inventory is the live post-Wave-83 record.
...
2. NPC fire is **cannon** plus Wave 83 **darts** for `pirate` + `ace` vs the player. NPCs do **not** emit `weapon: 'turret'`.
...
| `isTurretId` | Own-key, `SAFE_ID` length, reserved skip | `weapon-fit.js` 12–31, 67–69 |
...
| Tick | After player groups. Skip if destroyed / overheated. Docked already returned. | `combat.js` 1710–1713, 1831–1834 |
...
| Surface | Today | Cite |
|---|---|---|
| WPN copy | Groups 1–5 via `weaponHudLabel`. Turret is **not** a Digit and **not** on the WPN rail. | `hud.js` 196–229, 837–838, 1808 |
...
| Telegraph | ≥ 3 s before first shot. Demand-hold weapons cold. | `npc.js` 42–43, 1526–1538 |
| Turret emit | **Absent.** No `weapon: 'turret'`. | grep `npc.js` npcFire sites 1545–1548, 1921–1923 |
...
| Hangar | NPC shots do not call `spendMissileAmmo` / `writeMountedGear` | `combat.js` 1192; `hangar.js` 533 |
...
`testNpcHits`: skip shooter, skip Unknowable without consume, pad vs NPCs, stamp `lastAttacker` (`combat.js` 1538–1544, 1625–1626).

`lastAttacker` is instance-only, not saved (`npc.js` 231). Patrol hunt needs `lastAttacker === 'player'` (`npc.js` 1083–1090).

File header at `combat.js` 34–41 matches the live split (Wave 83 already rewrote the stale Wave 75 sentence). Code wins.
...
| World mirror | `ctx.world.turret` write-through of mounted row | `ctx.js` 171; `hangar.js` 271, 458, 480, 521–523 |
...
| `writeMountedGear` | Own-key patch; no `for-in` merge | `hangar.js` 505–525 |
...
| Chaff | **Absent.** No SKU. | inventory: no `chaff` in `WEAPONS` / `weapon-fit.js` |
...
| Surface | Today | Cite |
|---|---|---|
| HUD | `textContent` / `el()`. No `innerHTML` in `hud.js` combat HUD. | `hud.js` 239–244, 1103 |
...
NPC player-style `auto` is **absent**:

- No NPC seat check against `MOUNT_TABLE.turret`.
- No independent NPC turret clock.
- No NPC `TURRET_LIVE_CAP` split by `fromPlayer`.
- Wishlist still says “NPC turrets later” (`docs/PLAYER-EXPERIENCE-WISHLIST.md` 389–394). **Do not edit the wishlist.**

Wave 83 NPC darts do **not** make Wave 68 or Wave 83 incomplete.
...
Do **not** invent a new `WEAPONS` key. If a later owner wants different damage / ROF, that is a dedicated `state.js` PR **before** fire. Default: no `state.js` write.
