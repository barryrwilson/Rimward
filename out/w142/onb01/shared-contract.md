...
**Wave:** 142. This worker **implements PR1** (not markdown-only).  
**Status:** MERGE LAW for `docs/Onb01FlightLessonDesign.md`. If that document and this file ever disagree, **this file wins**.  
**Leftover:** **REAL.** Not CONSUME. Serial is **not** none. Named serial: **PR1** (sequential first-minute flight lesson + encyclopedia on demand).  
...
**Not this wave:** `src/game/state.js`, `origins.js`, pause (`main.js` KeyP / Ctl05), Hail01 / HUD-06 / Hail02 / HUD-07 / NAV-09 / NAV-10 governor / TGT-07 stills / MSN-04 other families / CTL-03 / AI-05 / CTL-04 / pad 2B / in-repo LLM. NAV-11 serial none. Do not edit `docs/PLAYER-EXPERIENCE-WISHLIST.md`, `PROGRESS.md`, `docs/Org01OriginPreviewDesign.md`, `docs/Ctl05PauseMenuDesign.md`. Do not steal sibling Wave 141 packs (Org01 origin preview, Ctl05 pause menu). Do not write `out/w142/onb01/verify/**`.
...
Integrator rule: Wave 142 PR1 obeys this file. Inventory cites live code at Wave 141 census. Code wins over the wishlist and over old playtest memory. If the design doc and this file disagree, **this file wins**.
...
1. This worker **implements PR1**. Partial merge is forbidden. Honor / fail-closed / deputize rules below **stay**. Do **not** steal sibling packs or expand the write-set.
...
| **WAVE6 harness** | Retarget `scripts/boot-test.mjs` **1719–1750** off id `move` / fragment `throttle` as the **first** card. First visible lesson card is **look**. Pin `ctx.world.origin = 'greenhand'` in **that SETUP only**. Do **not** rewrite other WAVE6 sections. |
...
- **Writer (harness only):** `scripts/boot-test.mjs` WAVE6 onboarding checks retarget.
...
First remaining serial is **PR1** and Wave 142 lands it.
