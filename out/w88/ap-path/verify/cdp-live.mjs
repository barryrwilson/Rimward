/**
 * Chrome CDP live AP-path check (Playwright MCP was locked).
 * Target: Vite http://localhost:5188/  Freehold → Veridian.
 */
...
if (!(already && already.ctx && String(already.href || '').includes('5189'))) {
  try {
  ...
  note('skip navigate, page already live');
}
...
const elapsed0 = await evalJson(`window.__ctx.elapsed`);
await sleep(700);
const elapsed1 = await evalJson(`window.__ctx.elapsed`);
note('elapsed ' + JSON.stringify({ elapsed0, elapsed1, ticking: elapsed1 > elapsed0 + 0.05 }));
...
try {
  engaged = await evalJson(`(() => {
    ...
    return {
      ...
      pos: { x: p.x, y: p.y, z: p.z },
      live: (document.getElementById('rw-galaxy-ap-live') || {}).textContent || '',
    };
  })()`);
} catch (err) {
...
for (let i = 0; i < 12; i++) {
  const s = await evalJson(`(() => {
    ...
    return {
      ...
      sys: ctx.world.currentSystem,
      yaw: ctx.autopilot && ctx.autopilot.yaw,
      pitch: ctx.autopilot && ctx.autopilot.pitch,
      throttle: ctx.autopilot && ctx.autopilot.throttle,
      wantJump: ctx.autopilot && ctx.autopilot.wantJump,
...
try {
  cons = await evalJson(`(() => {
    const ctx = window.__ctx;
    return {
      lastEvents: (ctx.lastEvents || []).slice(-8),
      reason: ctx.autopilot && ctx.autopilot.reason,
      flying: !!(ctx.world.nav && ctx.world.nav.autopilot),
      elapsed: ctx.elapsed,
    };
  })()`);
  note('end ' + JSON.stringify(cons));
} catch (err) {
  note('end fail ' + String(err));
}
...
const summary = {
  plotOk: !!(plotted && plotted.ok && plotted.next === 'veridian'),
